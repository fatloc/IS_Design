...
type Reaction =
  ...
  | "Không phù hợp"
  | "Muốn đặt cọc";
const REACTION_CFG: Record<
  Exclude<Reaction, "">,
  { icon: typeof Smile; color: string; bg: string; label: string }
> = {
  "Quan tâm cao": {
    icon: Smile,
    color: "#059669",
...
function parseSqlDate(value: any) {
...
function MiniCalendar({
  ...
  return (
    ...
    >
      ...
      <div
        className="grid grid-cols-7 px-2.5 pt-2"
      ...
      >
        {DAY_LABELS.map((d) => (
          <div
            ...
            style={{ fontSize: "0.62rem", fontWeight: 800, color: "#CBD5E1" }}
          >
            {d}
          </div>
        ))}
              ...
              >
                <span
                  style={{
                    fontSize: "0.78rem",
                    fontWeight:
                      day === selDay || isToday(day) || (dotMap[day] ?? 0) > 0
                        ? 800
                        : 400,
                    color:
                      day === selDay ? "white" : isToday(day) ? O : "#374151",
                  }}
                >
                  {day}
                </span>
                {(dotMap[day] ?? 0) > 0 && day !== selDay && (
                  <div className="absolute bottom-0.5 flex gap-0.5">
                    {Array.from({ length: Math.min(dotMap[day], 3) }).map(
                      (_, di) => (
                        <span
                          key={di}
      ...
      <div
        ...
        style={{ borderTop: "1px solid #F8FAFC", background: "#FAFBFD" }}
      >
    ...
    <div className="flex gap-3 mb-3">
      ...
      >
        <div
          ...
          style={{ background: status === "Đã hủy" ? "#F8FAFC" : "white" }}
        >
          <div
            className="w-9 h-9 rounded-full flex items-center justify-center text-white flex-shrink-0"
            style={{
              background: `linear-gradient(135deg,${O},#DC2626)`,
              fontWeight: 800,
              fontSize: "0.72rem",
            }}
          >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              >
                KH:{" "}
                {appt.khachHangXem
                  ? userMap.get(appt.khachHangXem) || appt.khachHangXem
                  : "--"}
              </div>
              {appt.isOverdue && (
                <span
                  className="flex items-center gap-1 text-red-600 px-1.5 py-0.5 rounded bg-red-50"
            ...
            <div className="flex items-center gap-3 mt-0.5 flex-wrap">
              ...
              <div className="flex items-center gap-1">
                ...
                <span style={{ fontSize: "0.72rem", color: "#94A3B8" }}>
                  NV:{" "}
                  {appt.nhanVienPhuTrach
                    ? userMap.get(appt.nhanVienPhuTrach) ||
                      appt.nhanVienPhuTrach
                    : "--"}
                </span>
              </div>
            ...
            {status !== "Đã hủy" && (
              <button
                ...
                style={{
                  ...
                  color: expanded ? O : "#64748B",
                  fontSize: "0.72rem",
                  fontWeight: 700,
                }}
              >
                <MessageSquare size={10} />
                {expanded ? "Đóng" : "Ghi chú"}
              </button>
            )}
          </div>
        </div>
        {expanded && (
          <div
            ...
            style={{ borderTop: `1px solid ${O}20`, background: `${O}04` }}
          >
            <div
              style={{ fontSize: "0.75rem", fontWeight: 800, color: "#374151" }}
            >
              Phản hồi khách hàng
            </div>
            <div className="flex flex-wrap gap-2">
              {(Object.keys(REACTION_CFG) as Exclude<Reaction, "">[]).map(
                (r) => {
            ...
            <textarea
              ...
              rows={2}
              placeholder="Ghi chú kết quả xem phòng..."
...
function PendingRequestsTab({ onScheduled }: { onScheduled: () => void }) {
  const {
    ...
    loading,
    reload,
  } = usePagedList<Request>(getRequests, 5000, {
  ...
  const handleSchedule = async () => {
    ...
    if (!selectedReq || !date || !time || !nvId || !roomId) {
      addToast({
        message: "Vui lòng điền đủ thông tin, bao gồm phòng cần xem",
        type: "error",
      });
      return;
    }
    ...
    if (nvId.length > 4) {
      addToast({
        message:
          "Mã nhân viên chỉ được chứa tối đa 4 ký tự (ví dụ: 0001). Vui lòng chọn đúng nhân viên từ danh sách!",
        type: "error",
      });
      return;
    }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
        <h3 className="font-bold text-slate-800 text-lg mb-4 flex items-center gap-2">
          <AlertTriangle size={18} className="text-orange-500" /> Danh sách yêu
          cầu cần lên lịch ({pendingRequests.length})
        </h3>
        <div className="mb-4">
          <input
            type="text"
            ...
            {paginatedRequests.map((req: Request) => (
              ...
              >
                <div className="font-bold text-slate-800 flex justify-between items-center">
                  ...
                  <span className="text-xs font-semibold px-2 py-1 bg-white border rounded-md text-slate-600">
                    Khách: {req.khachHangYeuCau}
                  </span>
                </div>
                <div className="text-sm text-slate-500 mt-2 flex gap-4">
                  ...
                  <span>
                    Ngân sách:{" "}
                    <strong className="text-slate-700">
                      {req.mucGiaMongMuon?.toLocaleString()}đ
                    </strong>
                  </span>
                </div>
                {req.cacTieuChiKhac && (
                  <div className="text-xs text-slate-400 mt-1 truncate">
                    Ghi chú: {req.cacTieuChiKhac}
                  </div>
                )}
              </div>
            ))}
            {pendingRequests.length === 0 && (
              <div className="bg-slate-50/50 border border-slate-200 border-dashed rounded-2xl p-8 text-center flex flex-col items-center justify-center mt-2">
                ...
                <p className="font-medium text-slate-600">
                  Hiện giờ không có yêu cầu mới cần lên lịch
                </p>
                <p className="text-xs text-slate-400 mt-1">
                  Các hồ sơ tạo mới từ mục Yêu cầu thuê sẽ hiển thị tại đây.
                </p>
              </div>
            )}
      ...
      <div>
        {selectedReq ? (
            ...
            <div className="space-y-5">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 border rounded-xl bg-slate-50 outline-none focus:bg-white focus:border-orange-500 transition"
                />
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1.5">
              ...
              <div>
                ...
                <datalist id="room-list">
                  {rooms.map((room) => (
                    <option
                      key={room.maPhong}
                      value={`${room.maPhong} - ${room.trangThai} - Giá: ${room.giaThuePhong}đ`}
                    />
                  ))}
                </datalist>
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1.5">
                  Mã NV phụ trách hướng dẫn (Sale)
                </label>
                <input
                  type="text"
                  list="staff-list"
                ...
                <datalist id="staff-list">
                  {employees.map((nv) => (
                    <option
                      key={nv.maNhanVien}
                      value={`${nv.maNhanVien} - ${nv.hoTen} - ${nv.soDienThoai}`}
                    />
                  ))}
                </datalist>
              </div>
              <button
                ...
                disabled={saving}
                className="w-full py-3.5 mt-4 bg-gradient-to-r from-orange-500 to-red-600 text-white font-bold rounded-xl shadow-lg shadow-orange-500/30 flex items-center justify-center gap-2 hover:brightness-110 transition disabled:opacity-50"
              >
                {saving ? (
                  "Đang xử lý..."
                ) : (
                  <>
                    <CheckCircle size={18} /> Xác nhận tạo lịch xem phòng
                  </>
                )}
              </button>
            </div>
          ...
          <div className="bg-slate-50 border-2 border-slate-200 border-dashed rounded-3xl p-10 text-center flex flex-col items-center justify-center h-full text-slate-500 min-h-[400px]">
            ...
            <p className="font-medium text-slate-600">
              Chọn một yêu cầu bên trái để bắt đầu lên lịch
            </p>
            <p className="text-sm mt-2">
              Hệ thống sẽ tự động chuyển yêu cầu sang trạng thái "Đã lên lịch
              xem".
            </p>
          </div>
        )}
...
export default function SaleAppointments() {
  ...
  // 1. Fetch appointments for the dots in MiniCalendar (whole month)
  const { items: monthAppointments } = usePagedList<Appointment>(getAppointments, 1000, {
    month: viewMonth + 1,
    year: viewYear,
  });

  // 2. Fetch appointments for the timeline (selected day)
  const {
    ...
    totalElements,
    reload,
  } = usePagedList<Appointment>(getAppointments, 100, {
    day: selDay,
    month: viewMonth + 1,
    year: viewYear,
  });
  ...
  const dayChorxem = dayAppointments.filter(
    (a) => a.trangThaiHen === "Chờ xác nhận",
  ).length;
  const dayDaxacnhan = dayAppointments.filter(
    (a) => a.trangThaiHen === "Đã xác nhận",
  ).length;
  const dayDaxem = dayAppointments.filter((a) => a.trangThaiHen === "Đã xem").length;
  const dayHuy = dayAppointments.filter((a) => a.trangThaiHen === "Đã hủy").length;
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          >
            Tiếp nhận yêu cầu mới, phân công nhân viên và theo dõi lịch hẹn xem
        ...
        >
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  >
                    {search
                      ? `Kết quả tìm kiếm`
                      : `Lịch trình: ${formattedSelDay}`}
                  </div>
                  <div className="flex items-center gap-3 mt-0.5">
                    {[
                      {
                        label: `${
                          search ? searchFiltered.length : dayAppointments.length
                        } cuộc hẹn`,
                        color: "#1E293B",
                      },
                      { label: `${dayChorxem} chờ xác nhận`, color: O },
                      {
                        label: `${dayDaxacnhan} đã xác nhận`,
                        color: "#4338CA",
                      },
            ...
            <div className="px-5 py-5 bg-white">
              {loading ? (
                ...
                <div className="text-center py-10">
                  <AlertTriangle
                    size={28}
                    style={{ color: "#DC2626", margin: "0 auto" }}
                  />
                ...
                <div className="flex flex-col items-center py-14">
                  ...
                  >
                    {search
                      ? "Thử từ khoá khác"
                      : "Chọn ngày khác trên lịch hoặc tạo lịch hẹn mới"}
                  </div>
                </div>
              ) : (
                <div>
            ...
            >
              ...
              <strong style={{ fontSize: "0.75rem", color: "#475569" }}>
                {totalElements.toLocaleString()} lịch hẹn
              </strong>
              <span style={{ fontSize: "0.75rem", color: "#CBD5E1" }}>·</span>
              <span style={{ fontSize: "0.75rem", color: "#94A3B8" }}>
                Chọn ngày trên lịch để lọc
              </span>
            </div>
          </div>
        </div>

...
import {
  monthlyRevenue, occupancyData,
  conversionData, contractExpirationData
} from "../data/mockData";
import { getTransactions } from "../services/api";
import type { SqlTransaction } from "../types";
...
function mapApiToUITransaction(apiTx: SqlTransaction): UITransaction {
  // Parsing hidden values from ghiChu if they exist (e.g. "Tiền phòng tháng 4 [AMOUNT:3500000] [CAT:Rent]")
  let parsedAmount = 1500000; // default mock if not found
  let parsedDesc = apiTx.ghiChu || "Giao dịch không tên";
  let parsedCat = "General";

  const amountMatch = parsedDesc.match(/\[AMOUNT:(\d+)\]/);
  if (amountMatch) {
    parsedAmount = Number(amountMatch[1]);
    parsedDesc = parsedDesc.replace(/\[AMOUNT:\d+\]/, "").trim();
  }

  const catMatch = parsedDesc.match(/\[CAT:([A-Za-z0-9_]+)\]/);
  if (catMatch) {
    parsedCat = catMatch[1];
    parsedDesc = parsedDesc.replace(/\[CAT:[A-Za-z0-9_]+\]/, "").trim();
  }

  return {
    id: apiTx.maPhieuThanhToan,
    type: apiTx.loaiGiaoDich?.includes("Thu") ? "Receipt" : "Payment",
    description: parsedDesc,
    amount: parsedAmount,
    date: apiTx.ngayGiaoDich || "2025-04-26",
    category: parsedCat,
    relatedTo: apiTx.maChungTu || "Không rành",
    status: apiTx.trangThai === "Completed" ? "Completed" : "Pending"
  };
}
...
function ReconciliationsView() {
  ...
  const [transactionsData, setTransactionsData] = useState<UITransaction[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    setIsLoading(true);
    getTransactions().then(res => {
      setTransactionsData((res.data || []).map(mapApiToUITransaction));
    }).catch(err => {
      console.error(err);
    }).finally(() => {
      setIsLoading(false);
  ...
  const filtered = transactionsData.filter(e => {
  ...
  const totalIncome = transactionsData.filter(e => e.type === "Receipt" && e.status === "Completed").reduce((a, e) => a + e.amount, 0);
  const totalPending = transactionsData.filter(e => e.type === "Receipt" && e.status === "Pending").reduce((a, e) => a + e.amount, 0);
  const totalExpense = transactionsData.filter(e => e.type === "Payment").reduce((a, e) => a + e.amount, 0);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        {isLoading ? (
           <div className="text-center py-10 text-slate-400">Đang tải biểu phí giao dịch...</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  {["Mô tả", "Loại", "Danh mục", "Ngày", "Chứng từ", "Số tiền", "Trạng thái", "Hành động"].map(h => (
              ...
              <tbody className="divide-y divide-slate-50">
                {filtered.map(e => (
                  <tr key={e.id} className="hover:bg-slate-50/60">
                    ...
                    <td className="px-4 py-3.5">
                       {e.status === "Pending" && (
                         <button className="px-2 py-1 bg-emerald-600 text-white rounded text-xs hover:bg-emerald-700 transition">
                           Xác nhận
                         </button>
                       )}
                    </td>
            ...
            {filtered.length === 0 && (
              <div className="text-center py-10 text-slate-400">Không tìm thấy giao dịch nào</div>
            )}
          </div>
        )}
      </div>

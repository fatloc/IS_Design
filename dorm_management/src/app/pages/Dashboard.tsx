...
import {
  ...
  Building2, BedDouble, ShieldCheck
} from "lucide-react";
import {
  contracts, rooms, userAccounts, pendingAccounts
} from "../data/mockData";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statusCards.map((card) => (
          <div key={card.label} className={`bg-white rounded-xl p-5 border border-slate-100 ring-1 ${card.ring} shadow-sm`}>
            ...
            <div className="text-xs text-slate-400 mt-0.5">/ {rooms.length} phòng tổng</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-xl p-5 border border-slate-100 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-slate-500">{rooms.length} phòng tổng cộng</span>
        </div>
        <div className="flex h-6 rounded-lg overflow-hidden gap-0.5">
          {statusCards.map((card) => (
            <div
              ...
              style={{ width: `${(card.count / rooms.length) * 100}%` }}
        ...
        <div className="flex gap-4 mt-3">
          {statusCards.map((card) => (
            <div key={card.label} className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-slate-500">{card.label} ({Math.round((card.count / rooms.length) * 100)}%)</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          ...
          <span className="text-xs text-slate-500">{contracts.length} hợp đồng</span>
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label: "Tổng hợp đồng", v: contracts.length, color: "text-slate-700", bg: "bg-white" },
          { label: "Đang hiệu lực", v: contracts.filter(c => c.status === "Active").length, color: "text-emerald-600", bg: "bg-white" },
          { label: "Sắp hết hạn", v: contracts.filter(c => c.status === "Expiring").length, color: "text-amber-600", bg: "bg-white" },
          { label: "Chờ duyệt", v: contracts.filter(c => c.status === "Pending").length, color: "text-blue-600", bg: "bg-white" },
        ].map(s => (
          ...
          {[
            ...
            <div key={section.title} className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
              <div className="flex items-center gap-2 mb-4">
                <div className={`w-8 h-8 bg-${section.color}-50 rounded-lg flex items-center justify-center`}>
